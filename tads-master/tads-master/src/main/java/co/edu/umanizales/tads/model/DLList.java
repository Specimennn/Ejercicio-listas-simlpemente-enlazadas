...
public class DLList {
    ...
    public void addToStart(Pet pet)throws DLListException{
        ...
        if (validAdd(pet) == false) {
            throw new DLListException("ya existe una mascota con ese nombre y ubicación");
    ...
    public void deleteByPlace (int place) {
        ...
        /*para cuando haga el controler de este metodo
        //tiro un error si la lista está vacia
        if (head == null){
            throw new DLListException("la lista está vacía");
        }

        if (place < 1) {
            throw new DLListException("el lugar no puede ser mwnor a 1");
        }
        */
    ...
    public void addPetAtPlace(@Valid Pet pet, int position) throws DLListException{
        ...
        if (position == 0){
            ...
            //si la cabeza no está vacía, lo meto antes de la cabeza
            if (head != null){
                newNode.setNext(head);
                head.setPrev(newNode);
            }

            //si la cabeza está vacía, lo defino como la cabeza
            head = newNode;

        }else {
            //aqui no lo estoy metiendo en la cabeza, entonces reocrro la lista hasta encontrarlo
            ...
            if (temp.getNext() != null){
                ...
                //aqui lo estaria metiendo en un lugar qeu no es el ultimo
